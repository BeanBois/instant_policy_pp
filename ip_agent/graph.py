...
def build_local_heterodata_single(
    ...
    scene_feats: Optional[Tensor] = None,  
) -> HeteroData:
    ...
    delta_as = scene_pos[ei_as[1]] - pos_a[ei_as[0]]         
...
def build_context_heterodata_single(
    curr_agent_emb: Tensor,       
    curr_agent_info: Tensor,      
    demo_agent_emb: Tensor,       
    ...
    data['curr'].x = curr_agent_emb          
    data['curr'].pos = curr_agent_pos.float()          
    demo_flat = demo_agent_emb.reshape(N * L * A, D)
    data['demo'].x = demo_flat                
    demo_pos_flat = demo_agent_pos.view(N*L*A,2)
    data['demo'].pos = demo_pos_flat.float()              
...
def build_action_heterodata_single(
    curr_agent_ctx: Tensor,   
    curr_agent_info: Tensor,  
    pred_agent_emb: Tensor,   
    pred_agent_info: Tensor,  
    num_freqs: int = 10,
) -> HeteroData:
