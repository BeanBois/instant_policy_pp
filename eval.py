...
def rollout_once(game_interface, agent, num_demos = 2, max_demo_length = 20, 
    ...
    while not done and _t < max_iter:
        ...
        for a0 in actions[0]: 
            ...
            curr_obs = game_interface.step(a0)
            done = curr_obs['done']
            if done:
                won = curr_obs['won']
                break
...
if __name__ == "__main__":
    ...
    from agent import GeometryEncoder
    import os 
    import argparse

    parser = argparse.ArgumentParser()
    parser.add_argument('--ip', action='store_true', help='pass flag to use instant policy')
    ...
    if ip :
        ...
        agent_state_dict = torch.load('ip-agent.pth', map_location="cpu")
